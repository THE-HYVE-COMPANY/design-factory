...
try {
  ...
  log("home has tabs", haveDsTab + haveSkillsTab > 0 ? "PASS" : "FAIL", `ds=${haveDsTab} skills=${haveSkillsTab}`);
  ...
  if (createDsCount > 0) {
    ...
    for (const b of allButtons.slice(0, 50)) {
      ...
      if (t && /forj|criar|\+/i.test(t)) {
        ...
        await page.waitForTimeout(500);
        if (await page.locator('text=/forjar.*design.*system/i').count() > 0) { opened = true; break; }
      }
    }
  ...
  await page.locator('button:has-text("design.md")').first().click({ timeout: 2000 }).catch(() => {});
  await page.waitForTimeout(200);
  ...
  log("DS engine chips render (Claude Code + sonnet)", (claudeChip > 0 && sonnetChip > 0) ? "PASS" : "FAIL", `claude=${claudeChip} sonnet=${sonnetChip}`);
  ...
  if (toggleCount > 0) {
    ...
    log("DS preview toggle changes state on click", initialState !== afterState ? "PASS" : "FAIL", `${initialState}→${afterState}`);
    // Restore ON
  ...
  // Close DS modal
  await page.locator('button[aria-label="Fechar"]').first().click({ timeout: 2000 }).catch(() => {});
  await page.waitForTimeout(300);
  ...
  const createSkillBtn = page.getByRole("button", { name: /nova skill|criar.*skill|novo.*skill/i }).first();
  ...
  for (const ph of [/minha skill/i, /\/minha-skill/i, /o que essa skill faz/i]) {
    createFields.push(await page.locator(`input[placeholder]`).filter({ has: page.locator(`[placeholder=""]`) }).count());
  }
  ...
  if (await nameField.count() > 0) {
    ...
    log("Skills create — trigger auto-slugs", triggerVal === "/test-validation-skill" ? "PASS" : "FAIL", `got: ${triggerVal}`);
  }
  ...
  await dialog.getByRole("button", { name: /^Pasta$/i }).first().click({ timeout: 2000 }).catch(() => {});
  await page.waitForTimeout(200);
  const folderInput = dialog.locator('input[placeholder*="skills"]').first();
  log("Skills import Pasta source — input renders", await folderInput.count() > 0 ? "PASS" : "FAIL");
  ...
  if (detailOpened) {
    // Check fields
    log("Skill detail — descrição field", await page.locator('input[placeholder*="quando usar"]').count() > 0 ? "PASS" : "FAIL");
    log("Skill detail — corpo textarea", await page.locator('textarea').count() > 0 ? "PASS" : "FAIL");

    // ★ THE NEW MULTIFILE SECTION ★
    const filesSection = page.locator('text=/arquivos.*da.*skill/i');
    log("Skill detail — multifile section visible", await filesSection.count() > 0 ? "PASS" : "FAIL");
    ...
    log("Skill detail — Excluir button", await page.locator('button:has-text("Excluir")').count() > 0 ? "PASS" : "FAIL");
    log("Skill detail — Exportar .md button", await page.getByRole("button", { name: /exportar.*\.md/i }).count() > 0 ? "PASS" : "FAIL");
    log("Skill detail — Salvar button", await page.locator('button:has-text("Salvar")').count() > 0 ? "PASS" : "FAIL");

    // Salvar should be disabled (not dirty yet)
    const saveBtn = page.locator('button:has-text("Salvar")').first();
    log("Skill detail — Salvar disabled before edit", await saveBtn.isDisabled() ? "PASS" : "FAIL");
    ...
    if (await ta.count() > 0) {
      ...
      await page.waitForTimeout(200);
      log("Skill detail — Salvar enables after edit", !(await saveBtn.isDisabled()) ? "PASS" : "FAIL");
    }
  }
