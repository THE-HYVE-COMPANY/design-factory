...
// Optional: inject window.__DF_BRIDGE_PORT__ for prod-build (vite preview)
// testing where VITE_BRIDGE_URL was baked at build time.
const BRIDGE_PORT = process.env.DF_INJECT_BRIDGE_PORT ? Number(process.env.DF_INJECT_BRIDGE_PORT) : null;
...
async function runOneProvider(browser, providerId, seedPrompt, scenario = "seed") {
  ...
  try {
    ...
    // When testing the production build via `vite preview`, VITE_BRIDGE_URL
    // was baked at build time (defaulting to :1421). Inject the actual bridge
    // port so resolveBridgeUrl() (which checks window.__DF_BRIDGE_PORT__ first)
    // points at our standalone daemon. No-op for dev:web (env var already set).
    if (BRIDGE_PORT) {
      await page.addInitScript((port) => {
        try { window.__DF_BRIDGE_PORT__ = port; } catch {}
      }, BRIDGE_PORT);
    }
