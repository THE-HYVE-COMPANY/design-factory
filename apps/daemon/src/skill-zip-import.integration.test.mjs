...
describe("skill ZIP import round-trip", () => {
  it("installs the minimal skill zip into skills/<zip-folder>/ with sibling files intact", async () => {
    const zip = await readFile(resolve(process.cwd(), "examples/fixtures/skills/minimal-skill.zip"));
    ...
    const reference = await readFile(join(repoRoot, "skills", "minimal-skill", "references", "style.md"), "utf8");
