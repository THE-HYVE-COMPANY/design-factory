...
const claude = {
  ...
  async stream(req, res, deps) {
    ...
    const spawnOpts = cwd && typeof cwd === "string" ? { stdio: ["pipe", "pipe", "pipe"], cwd } : { stdio: ["pipe", "pipe", "pipe"] };
    ...
    child.on("close", () => { cleanupSpool(); });
    child.on("error", () => { cleanupSpool(); });
  ...
  async once(req, res, deps) {
    ...
    const onceSpawnOpts = cwd && typeof cwd === "string" ? { stdio: ["pipe", "pipe", "pipe"], cwd } : { stdio: ["pipe", "pipe", "pipe"] };
    if (cwd) ensureGitRepo?.(cwd);
    ...
    child.on("error", () => { cleanupOnceSpool(); });
  },
};
