...
const opencode = {
  ...
  async stream(req, res, deps) {
    ...
    const OPENCODE_BIN = process.env.DF_OPENCODE_BIN || "opencode";
    // opencode 1.15+ flags:
    ...
    const child = spawn(OPENCODE_BIN, args, spawnOpts);
  ...
  async once(req, res, deps) {
    ...
    const composed = systemPrompt ? `${systemPrompt}\n\n---\n\n${prompt}` : prompt;
    args.push(composed);
    const onceSpawnOpts = { stdio: ["ignore", "pipe", "pipe"], shell: process.platform === "win32" };
    if (cwd) onceSpawnOpts.cwd = cwd;
    const child = spawn(OPENCODE_BIN, args, onceSpawnOpts);
