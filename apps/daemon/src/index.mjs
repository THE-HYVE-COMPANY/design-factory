...
const DEFAULT_ALLOWED_ORIGINS = [
  ...
  // :3000 (Next/Tauri preview) while the daemon was rejecting CORS.
  "http://localhost:3000",
  "http://127.0.0.1:3000",
  "http://localhost:5173",
  "http://127.0.0.1:5173",
  "tauri://localhost",       // Tauri webview origin (macOS)
  "http://tauri.localhost",  // Tauri WebView2 origin (Windows — confirmed on a real build, http)
  "https://tauri.localhost", // Tauri origin (https variant)
];
...
function buildSkill({ raw, absPath, source, cwd }) {
  ...
  const name = parsed.name || basename.replace(/[-_]/g, " ").replace(/^\w/, (c) => c.toUpperCase());
...
async function updateDfSkill(id, patch) {
  ...
  const canonicalDir = getSkillsDir();
  const legacyDir = getLegacySkillsDir();
  const allowed = filePath.startsWith(canonicalDir + "/") || filePath.startsWith(legacyDir + "/");
...
async function deleteDfSkill(id) {
  ...
  const canonicalDir = getSkillsDir();
  const legacyDir = getLegacySkillsDir();
  const allowed = dir.startsWith(canonicalDir + "/") || dir.startsWith(legacyDir + "/");
  ...
  if (req.method === "POST" && req.url === "/gh/device/poll") {
    try {
      ...
      if (data.access_token) {
        ...
        try {
          const parent = DF_TOKEN_PATH.replace(/\/[^/]+$/, "");
  ...
  async function describeWithAvailability(p) {
    ...
    try {
      ...
      if (p.id === "anthropic") {
        ...
        // Optimistic — real probe lives at GET /ollama/models.
        available = true;
      }
    } catch {
      available = false;
