...
│   ├── screens/            # Top-level routes (Home, Editor, Settings, DsPreview, Showcase, Shaders, Dev)
...
We follow [Conventional Commits][cc] with an extra trailer:
...
Agent-Id: your-handle
```

Prefixes we use: `feat`, `fix`, `refactor`, `docs`, `chore`, `test`.
...
reproduce. For visual changes touching atoms, run
`npm run test:visual` and update snapshots when intentional.
