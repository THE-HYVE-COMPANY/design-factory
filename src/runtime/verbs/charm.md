...
id: charm
label: Charm
description: Add personality and memorable touches
category: direction
hue: warm-coral
modifiesHtml: true
icon: heart
...
4. **One signature element** — one detail that distinguishes this design from a thousand similar ones. A custom underline on links. A non-standard cursor on a specific element. An asymmetric heading layout. A typographic flourish on the brand wordmark.
5. **Numerals as visual elements** — if the design has stats or numbers, treat them as typography (large, weight-confident, deliberate placement) rather than information density.
