...
// Experimental + dev-only screens (new-project labs, showcases, shader
// gallery, dev harness). Lazy-loaded and gated behind import.meta.env.DEV
// in the route table below, so they are dropped from the production bundle
// and unreachable in the public build. Keep them out of the static import
// graph above — that is what lets Vite tree-shake them in prod.
const DevScreen = lazy(() => import("@/screens/DevScreen").then((m) => ({ default: m.DevScreen })));
const ShowcaseScreen = lazy(() => import("@/screens/ShowcaseScreen").then((m) => ({ default: m.ShowcaseScreen })));
const SkeuShowcaseScreen = lazy(() => import("@/screens/SkeuShowcaseScreen").then((m) => ({ default: m.SkeuShowcaseScreen })));
const ShadersScreen = lazy(() => import("@/screens/ShadersScreen").then((m) => ({ default: m.ShadersScreen })));
const NewProjectLabScreen = lazy(() => import("@/screens/NewProjectLabScreen").then((m) => ({ default: m.NewProjectLabScreen })));
const NewProjectRegionsLabScreen = lazy(() => import("@/screens/NewProjectRegionsLabScreen").then((m) => ({ default: m.NewProjectRegionsLabScreen })));
const NewProjectPromptFirstLab = lazy(() => import("@/screens/NewProjectPromptFirstLab").then((m) => ({ default: m.NewProjectPromptFirstLab })));
const NewProjectLabsHub = lazy(() => import("@/screens/NewProjectLabsHub").then((m) => ({ default: m.NewProjectLabsHub })));
const NewProjectConvLab = lazy(() => import("@/screens/NewProjectConvLab").then((m) => ({ default: m.NewProjectConvLab })));
const NewProjectMoodLab = lazy(() => import("@/screens/NewProjectMoodLab").then((m) => ({ default: m.NewProjectMoodLab })));
const NewProjectVerbLab = lazy(() => import("@/screens/NewProjectVerbLab").then((m) => ({ default: m.NewProjectVerbLab })));
const NewProjectSpatialLab = lazy(() => import("@/screens/NewProjectSpatialLab").then((m) => ({ default: m.NewProjectSpatialLab })));
const NewProjectCanonicalPlusLab = lazy(() => import("@/screens/NewProjectCanonicalPlusLab").then((m) => ({ default: m.NewProjectCanonicalPlusLab })));
  ...
  return (
    ...
    <div style={{ height: "100%", position: "relative" }}>
      ...
      <Routes>
        ...
        {/* Experimental + dev-only routes. Registered only in dev builds
            (import.meta.env.DEV); in the production/public build these
            screens are tree-shaken out and the paths fall through to the
            catch-all redirect below. */}
        {import.meta.env.DEV && (
          <>
            <Route path="/dev" element={<DevRoute />} />
            <Route path="/showcase" element={<ShowcaseScreen theme={theme} onThemeChange={handleThemeChange} />} />
            <Route path="/showcase/skeu" element={<SkeuShowcaseScreen onBack={() => window.history.back()} />} />
            <Route path="/skeu" element={<SkeuShowcaseScreen onBack={() => window.history.back()} />} />
            <Route path="/shaders" element={<ShadersScreen />} />
            <Route path="/new-project-lab" element={<NewProjectLabScreen />} />
            <Route path="/lab" element={<NewProjectLabScreen />} />
            <Route path="/lab-regions" element={<NewProjectRegionsLabScreen />} />
            <Route path="/regions" element={<NewProjectRegionsLabScreen />} />
            <Route path="/np-prompt-first" element={<NewProjectPromptFirstLab />} />
            <Route path="/np-b" element={<NewProjectConvLab />} />
            <Route path="/np-c" element={<NewProjectMoodLab />} />
            <Route path="/np-d" element={<NewProjectVerbLab />} />
            <Route path="/np-e" element={<NewProjectSpatialLab />} />
            <Route path="/np-canonical-plus" element={<NewProjectCanonicalPlusLab />} />
            <Route path="/np-hub" element={<NewProjectLabsHub />} />
          </>
        )}
...
function DevRoute() {
  const navigate = useNavigate();
  return <DevScreen onExit={() => navigate("/")} />;
}
