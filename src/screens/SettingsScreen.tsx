        ...
        <main className="settings-main">
          ...
          {nav === "appearance" && (
            <section className="settings-page appearance-panel" aria-label={t("settings.appearance.title")}>
              <div className="settings-kicker">{t("settings.appearance.kicker")}</div>
          ...
          {nav === "shortcuts" && (
            <section className="settings-page" aria-label={t("settings.shortcuts.title")}>
              <div className="settings-kicker">{t("settings.shortcuts.kicker")}</div>
              <h1 className="settings-title">{t("settings.shortcuts.title")}</h1>

              <section className="settings-group" style={{ borderTop: 0, paddingTop: 0 }}>
                <h2 className="settings-group-title">{t("settings.shortcuts.section.chat")}</h2>
                {[
                  { label: t("settings.shortcuts.chat.send"),        kbd: "↵" },
                  { label: t("settings.shortcuts.chat.newline"),     kbd: "⇧ ↵" },
                  { label: t("settings.shortcuts.chat.cancel"),      kbd: "Esc" },
                  { label: t("settings.shortcuts.chat.selectslash"), kbd: "↑ / ↓ · ↵" },
                  { label: t("settings.shortcuts.chat.insertargs"),  kbd: "Tab" },
                  { label: t("settings.shortcuts.chat.closeslash"),  kbd: "Esc" },
                ].map((s) => (
              ...
              <section className="settings-group">
                <h2 className="settings-group-title">{t("settings.shortcuts.section.comments")}</h2>
                {[
                  { label: t("settings.shortcuts.comments.save"),   kbd: "⌘ ↵" },
                  { label: t("settings.shortcuts.comments.cancel"), kbd: "Esc" },
                ].map((s) => (
                  <ShortcutRow key={s.label} {...s} />
                ))}
              </section>
              ...
              <section className="settings-group">
                <h2 className="settings-group-title">{t("settings.shortcuts.section.modals")}</h2>
                {[
                  { label: t("settings.shortcuts.modals.confirmsave"),   kbd: "↵" },
                  { label: t("settings.shortcuts.modals.confirmtweaks"), kbd: "⌘ ↵" },
                  { label: t("settings.shortcuts.modals.dismiss"),       kbd: "Esc" },
                ].map((s) => (
                  <ShortcutRow key={s.label} {...s} />
                ))}
              </section>
              ...
              <section className="settings-group">
                <h2 className="settings-group-title">{t("settings.shortcuts.section.canvas")}</h2>
                <p className="settings-group-sub" style={{ marginBottom: 8 }}>
                  {t("settings.shortcuts.canvas.body")}
                </p>
              </section>
            </section>
          )}
          ...
          {nav === "about" && (
            <section className="settings-page" aria-label={t("settings.about.title")}>
              <div className="settings-kicker">{t("settings.about.kicker")}</div>
              <h1 className="settings-title">{t("settings.about.title")}</h1>
              <section className="settings-group" style={{ borderTop: 0, paddingTop: 0 }}>
                <p style={{ fontSize: "var(--df-text-sm)", color: "var(--df-text-secondary)", lineHeight: 1.6, marginBottom: "var(--df-sp-5)" }}>
                  {t("settings.about.body")}
                </p>
                <div style={{ display: "flex", flexDirection: "column", gap: "var(--df-sp-2)" }}>
                  {[
                    { label: t("settings.about.field.version"),   value: "0.1.0" },
                    { label: t("settings.about.field.license"),   value: "MIT" },
                    { label: t("settings.about.field.stack"),     value: "React 18 + Vite + Node daemon" },
                    { label: t("settings.about.field.runtime"),   value: t("settings.about.value.runtime") },
                    { label: t("settings.about.field.providers"), value: "Claude · Codex · Gemini · OpenCode · Kimi" },
                    { label: t("settings.about.field.publish"),   value: "Vercel BYOK" },
                  ].map((item) => (
                    <div key={item.label} style={{ display: "flex", justifyContent: "space-between", fontSize: "var(--df-text-sm)", padding: "6px 0", borderBottom: "1px solid var(--df-border-subtle)" }}>
                      <span style={{ color: "var(--df-text-muted)" }}>{item.label}</span>
                      <span style={{ fontFamily: "var(--df-font-mono)", fontSize: "var(--df-text-xs)", color: "var(--df-text-secondary)" }}>{item.value}</span>
                    </div>
                  ))}
                </div>
              </section>
...
function ShortcutRow({ label, kbd }: { label: string; kbd: string }) {
  return (
    <div style={{
...
function ProvidersPanel({ defaultProvider, providerStatus, probing, onRefresh, onSetDefault }: ProvidersPanelProps) {
  ...
  return (
    <section className="settings-page" aria-label={t("settings.providers.title")}>
      <div className="settings-kicker">{t("settings.providers.kicker")}</div>
