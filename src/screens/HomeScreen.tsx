...
export function HomeScreen({
  ...
  const handleRemoveDs = async (path: string, opts: { skipConfirm?: boolean } = {}) => {
    const entry = designSystems.find((d) => d.path === path);
    const name = entry?.name || "this design system";
    if (!opts.skipConfirm) {
      if (!window.confirm(`Delete "${name}" from disk? The design-systems/<slug> folder will be removed — can't be undone.`)) return;
    }
  ...
  useEffect(() => {
    const handler = (ev: Event) => {
      ...
      if (detail?.path) handleRemoveDs(detail.path, { skipConfirm: true });
    };
  ...
  useEffect(() => {
    let cancelled = false;
    const idsLoaded = Object.keys(projectHtmls);
    const toLoad = recentProjects.filter((p) => !idsLoaded.includes(p.id));
    if (toLoad.length === 0) return;
    (async () => {
      for (const p of toLoad) {
        const folder = p.path.replace(/[\\/]+$/, "");
        const slug = slugFromPath(folder) || p.id;
        // Fast path: assume {slug}.html (the convention for projects
        // created from "New project" where the name maps to the folder
        // slug). User repro 2026-05-21: untitled-* projects break
        // this — folder is "untitled-XYZW" but the file inside is just
        // "untitled.html" (the suffix is appended to the folder only,
        // not the file). Fall back to listing the folder + picking the
        // first .html when the fast path returns nothing.
        ...
        if (cancelled) return;
        setProjectHtmls((prev) => ({ ...prev, [p.id]: html ?? "" }));
      }
    })();
  ...
  useEffect(() => {
    let cancelled = false;
    const idsLoaded = Object.keys(projectRatios);
    const toLoad = recentProjects.filter((p) => !idsLoaded.includes(p.id));
    if (toLoad.length === 0) return;
    (async () => {
      for (const p of toLoad) {
        ...
        setProjectRatios((prev) => ({ ...prev, [p.id]: ratio }));
      }
    })();
    return () => { cancelled = true; };
  }, [recentProjects]); // eslint-disable-line react-hooks/exhaustive-deps
                ...
                <div className="home-right-grid">
                  {designSystems.map((ds) => (
                    <EntityCard
                      ...
                      subtitle={ds.source === "github" && ds.sourceRef
                        ? `⎇ ${ds.sourceRef}`
                        : ds.path.split("/").slice(-2).join("/")}
                      subtitleMono
                      hoverTitle={ds.path}
                      ...
                      actions={[
                        ...
                        },
                        {
                          ...
                          onSelect: () => handleRemoveDs(ds.path),
                        },
                      ]}
                    />
                  ))}
                  ...
                  <EntityCard
                    ...
                    actions={[
                        ...
                        : []),
                      {
                        ...
                        tone: "danger" as const,
                        onSelect: async () => {
                          if (!window.confirm(tf("home.project.delete.confirm", p.name))) return;
                          try {
                            await onDeleteProject(p.id);
                          } catch (err) {
                            console.error("[project] delete failed", err);
                            window.alert(
                              tf("home.project.delete.failed", p.name, err instanceof Error ? err.message : String(err)),
                            );
                          }
                        },
                      },
          ...
          {filtered.map((s) => {
            ...
            if (!isReadOnly) {
              ...
              actions.push({
                ...
                tone: "danger" as const,
                onSelect: async () => {
                  if (!window.confirm(`Apagar a skill "${s.name}" permanentemente?`)) return;
                  const ok = await deleteSkill(s.id);
                  if (ok) {
                    flashToast(tf("home.skills.toast.deleted"));
                    await rescan();
                  } else {
                    flashToast("Falha ao apagar skill");
                  }
                  setMenuOpenId(null);
                },
              });
...
function DsCardPreview({ designMdPath, coverPath }: { designMdPath: string; coverPath?: string }) {
  ...
  const [state, setState] = useState<"loading" | "ok" | "empty" | "missing">("loading");
  ...
  useEffect(() => {
    ...
    readFileViaBridge(designMdPath).then((f) => {
      ...
      if (content.length < 40) { setState("empty"); return; }
      try {
        ...
        if (hex.length > 0) { setColors(hex); setState("ok"); }
        else { setState("empty"); }
      } catch {
        setState("empty");
      }
        ...
        <div className="home-right-grid">
          {templates.map((tpl) => (
            <EntityCard
              ...
              subtitle={`~${Math.round(tpl.html.length / 1024)}KB`}
              subtitleMono
