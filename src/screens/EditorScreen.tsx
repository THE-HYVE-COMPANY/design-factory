...
import { TurnTimelinePanel } from "@/components/TurnTimelinePanel";
import { buildTweaksPrompt, parseTweaksResponse, TWEAKS_SYSTEM_PROMPT, workspaceContextPreamble, invokeConsult, looksLikeQuestion, type ProjectContext } from "@/runtime/prompt-invoker";
...
import { type CanonicalPlusInput, type DialDirection, type DialKey } from "@/runtime/canonical-plus-prompt";
...
export function EditorScreen({ projectId, projectName, projectPath, mode, startMode = "prototype", initialPrompt, theme, onThemeChange, onHome, onOpenSettings, onDuplicateProject }: EditorScreenProps) {
  ...
  const [attachedFiles, setAttachedFiles] = useState<Array<{ name: string; size: number; content: string; mime: string }>>([]);
        ...
        if (projectId) {
          ...
          if (raw) {
            ...
            if (parsed?.designSystem && typeof parsed.designSystem === "string") {
              ...
              const slug = parsed.designSystem.split("/").filter(Boolean).pop() ?? null;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (loaded && loaded.length > 0) {
        const { messages: sanitized } = sanitizeMessages(loaded);
        setMessages(sanitized);
        persistedTurnIdsRef.current = writtenTurnIds;
      } else if (projectSlug) {
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    if (verbHit) {
      if (!iframeHtml) {
        showToast("Generate a design first");
        return;
      }
      setInput("");
    ...
    if (iframeHtml) {
      ...
      if (addMatch) {
        ...
        if (applied && "applied" in applied && applied.html) {
          ...
          if (projectPath) {
            const slug = slugFromPath(projectPath) || projectId || "untitled";
            const filePath = `${projectPath.replace(/\/$/, "")}/${slug}.html`;
  ...
  const handleAttach = useCallback(async (files: FileList | null) => {
    ...
    const newOnes: Array<{ name: string; size: number; content: string; mime: string }> = [];
    for (const f of Array.from(files)) {
      ...
      if (isImage) {
        if (!projectPath) { showToast("Save the project first"); continue; }
        if (f.size > 5 * 1024 * 1024) { showToast(`${f.name} too large (>5MB)`); continue; }
        const buf = await f.arrayBuffer();
        const base64 = btoa(String.fromCharCode(...new Uint8Array(buf)));
        const safeName = `${Date.now().toString(36)}-${f.name.replace(/[^a-zA-Z0-9._-]/g, "_")}`;
        ...
        newOnes.push({ name: f.name, size: f.size, content: diskPath, mime });
      } else {
        ...
        else content = `data:${mime};base64,${btoa(String.fromCharCode(...new Uint8Array(await f.arrayBuffer())))}`;
  ...
  const handleSaveVersion = useCallback(() => {
    if (!iframeHtml) { showToast("Nothing to save yet"); return; }
  ...
  const handleRestoreFromTurn = useCallback((versionId: string) => {
    ...
    if (!v) {
      showToast("Snapshot not found — it may have been pruned.");
      return;
    }
  ...
  const handleGenerateTweaks = useCallback(async () => {
    ...
    if (!source) {
      showToast("Open an HTML tab first — Tweaks operates on visible HTML");
      return;
    }
  ...
  const handleJumpToComment = useCallback((c: Comment) => {
    ...
    try {
      ...
      if (!el) { showToast("Element not found in current preview"); return; }
  ...
  return (
    <div className="screen" data-active="true">
      ...
      <div className="editor">
        ...
        >
          ...
          <div className="topbar-left">
            <button className="topbar-home" title="All projects" onClick={onHome}>
              <Logo size={16} />
            </button>
                        ...
                        <span style={{
                          ...
                          <button
                            onClick={() => {
                              ...
                              showToast("Design system detached");
                            }}
                    ...
                    <div className="chat-input-bar-left">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          showToast("Realtime dictation needs Chrome/Edge/Safari");
                        }}
            ...
            <div className="canvas-toolbar">
                  ...
                  {showTweaksRequest && (
                    ...
                    >
                      ...
                      <textarea
                        autoFocus
                        className="chat-input-field"
                        placeholder="e.g. colors, spacing, radius, font size..."
                        value={tweaksRequest}
              ...
              <div style={{ marginLeft: "auto", display: "flex", gap: "var(--df-sp-2)" }}>
                <button
                  ...
                  onClick={() => {
                    if (!iframeHtml) { showToast("Generate a design first"); return; }
                    setPresenting(true);
                  }}
                  disabled={!iframeHtml}
        ...
        <div className="cmd-overlay" onClick={() => setShowCmdPalette(false)}>
          <div className="cmd-modal" onClick={(e) => e.stopPropagation()}>
            <div className="cmd-search">
              ...
              <input className="cmd-search-input" placeholder="Search commands, files..." autoFocus />
      ...
      {/* Live turn timeline panel — observability overlay. Auto-shows on
        * first turn, dismissible via Ctrl+Shift+T or × button. Records SSE
        * + tool + iframe + artifact + persist events with millisecond
        * timing so the user can see which step broke without spelunking
        * console logs. Persists to .df/sessions/{turnId}.jsonl. */}
      <TurnTimelinePanel projectId={projectId} />
    </div>
  );
}
