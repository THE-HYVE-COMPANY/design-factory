...
const MOCK_SKILLS: ReadonlyArray<MockSkill> = [
  { id: "make-interfaces-feel-better", name: "Make Interfaces Feel Better", trigger: "/make-interfaces", description: "Polish + micro-details on any UI", iconHint: "polish" },
  { id: "audit-design-system",         name: "Audit Design System",         trigger: "/audit-ds",        description: "Token counting + drift check",      iconHint: "audit" },
  { id: "translate-into-portuguese",   name: "Translate Into Portuguese",   trigger: "/translate-pt",    description: "Locale + tone-aware translation",   iconHint: "transform" },
  { id: "research-competitors",        name: "Research Competitors",        trigger: "/research",        description: "Surface + summarize competitors",   iconHint: "research" },
];
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        color: "var(--df-text-primary)", opacity: 0.92,
        lineHeight: 1,
      }}>{letter}</span>
      <span style={{
...
function CoverF_ColorBlock({ skill }: { skill: MockSkill }) {
  ...
  return (
    <div style={{
      width: "100%", height: "100%",
      background: bg, color: "rgba(255,255,255,0.85)",
  ...
  return (
    <div style={{
      ...
      <header style={{ marginBottom: 24, maxWidth: 1100 }}>
        <div style={{ fontFamily: "var(--df-font-mono)", fontSize: 11, color: "var(--df-text-faint)", letterSpacing: "0.18em", textTransform: "uppercase" }}>
          Preview · Skill covers
        </div>
        <h1 style={{ fontSize: 28, fontWeight: 600, letterSpacing: "-0.02em", margin: "6px 0 8px" }}>
          6 direções, 4 skills mockadas cada
        </h1>
        <p style={{ fontSize: 14, color: "var(--df-text-secondary)", lineHeight: 1.55, margin: 0 }}>
          Cada coluna é uma direção visual. Cada linha é a mesma skill renderizada em todas. Olha o conjunto e me diz qual letra (A-F).
        </p>
      </header>
      ...
      <div style={{
        ...
        {/* Column headers */}
        {OPTIONS.map((opt) => (
          <div key={`h-${opt.id}`} style={{
            fontFamily: "var(--df-font-mono)", fontSize: 10,
            color: "var(--df-text-faint)", letterSpacing: "0.12em",
            textTransform: "uppercase",
            paddingBottom: 4, borderBottom: "1px solid var(--df-border-subtle)",
          }}>
            {opt.id} · {opt.label}
          </div>
        ))}
            ...
            return (
              ...
              >
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: 13, fontWeight: 500, color: "var(--df-text-primary)", lineHeight: 1.25, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {skill.name}
                  </div>
                  <div style={{
                    fontFamily: "var(--df-font-mono)",
                    fontSize: 11, color: "var(--df-text-faint)",
      ...
      <footer style={{
        ...
        background: "var(--df-bg-section)",
        border: "1px solid var(--df-border-subtle)",
        fontSize: 12, color: "var(--df-text-muted)", lineHeight: 1.5,
        maxWidth: 1100,
      }}>
        Manda a letra (A, B, C, D, E ou F) — se quiser combinar (e.g.
        "C com cor de B"), descreve que aplico.
      </footer>
    </div>
  );
}
