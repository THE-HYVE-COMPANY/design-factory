...
// Bridge URL is owned by claude-bridge — import it directly. The previous
// inline copy tried to "re-derive" but checked the wrong window key
// (`__DF_BRIDGE_URL__` instead of `__DF_BRIDGE_PORT__`) and ignored
// `import.meta.env.VITE_BRIDGE_URL`, so every fetch defaulted to
// http://localhost:1421 even when dev-web reclaimed a different port —
// causing indefinite hangs when port 1421 was held by another process
// (e.g. VS Code) that doesn't speak HTTP. No import cycle: claude-bridge
// does not import provider-sessions.
