...
describe("[gate] Durable Chat Journal — auditor verdict 2026-05-08", () => {
  ...
  // four canonical origins, handles null/empty) lives in
  ...
  it("[Fase 1 #3] origin-guard exposes canonical origins and is mounted in App", () => {
    ...
    expect(guard).toMatch(/tauri:\/\/localhost/);
    // Daemon parity — the four canonical origins must match the daemon's
    ...
    expect(daemon).toMatch(/DEFAULT_ALLOWED_ORIGINS\s*=\s*\[[\s\S]*?http:\/\/localhost:1420[\s\S]*?http:\/\/127\.0\.0\.1:1420[\s\S]*?tauri:\/\/localhost[\s\S]*?https:\/\/tauri\.localhost/);
