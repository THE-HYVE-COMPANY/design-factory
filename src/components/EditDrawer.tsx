...
export const EDIT_PRESETS: Array<{ id: string; label: string; values: EditOverrides }> = [
  ...
  { id: "paper",   label: "Paper",   values: { bg: "#f4f1ea", fg: "#1a1a17", font: "'Iowan Old Style', Georgia, serif", baseFs: 17 } },
  { id: "cyber",   label: "Cyber",   values: { bg: "#050816", fg: "#c5ffee", font: "ui-monospace, 'SF Mono', Menlo, Consolas, monospace", radius: 0 } },
  { id: "pastel",  label: "Pastel",  values: { bg: "#fff8f0", fg: "#3a2c1f", radius: 20, shadow: 0.08 } },
  { id: "mono",    label: "Mono",    values: { bg: "#fff", fg: "#000", font: "ui-monospace, 'SF Mono', Menlo, Consolas, monospace", radius: 0 } },
    ...
    >
      <div style={{
        ...
        <button
          onClick={onClose}
          title="Close"
          style={{ padding: "2px 6px", color: "var(--df-text-faint)", fontSize: 14, cursor: "pointer" }}
        >✕</button>
      </div>

      <div style={{ flex: 1, overflowY: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 14 }}>
        <div>
          <div style={{ fontSize: 10, fontFamily: "var(--df-font-mono)", color: "var(--df-text-muted)", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>
            Presets
          </div>
          <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
            {EDIT_PRESETS.map((preset) => (
              <button
                key={preset.id}
                onClick={() => { setLocal(preset.values); onChange(preset.values); }}
                style={{
                  padding: "4px 10px",
                  fontSize: 10,
                ...
                onMouseLeave={(e) => { e.currentTarget.style.background = "var(--df-surface-raised)"; e.currentTarget.style.color = "var(--df-text-secondary)"; }}
              >
                {preset.label}
              </button>
            ))}
        ...
        <Row label="Background" value={local.bg ?? ""}>
          <ColorBinding value={local.bg ?? ""} onChange={(v) => update({ bg: v || undefined })} placeholder="inherit" />
        </Row>

        <Row label="Text color" value={local.fg ?? ""}>
          <ColorBinding value={local.fg ?? ""} onChange={(v) => update({ fg: v || undefined })} placeholder="inherit" />
        </Row>

        <Row label="Page padding" value={local.pad !== undefined ? `${local.pad}px` : ""}>
          <Slider min={0} max={120} step={2} value={local.pad ?? 0} onChange={(v) => update({ pad: v })} />
        </Row>

        <Row label="Max content width" value={local.maxW ? `${local.maxW}px` : "none"}>
          <Slider min={0} max={1600} step={20} value={local.maxW ?? 0} onChange={(v) => update({ maxW: v || undefined })} />
        </Row>

        <Row label="Base font size" value={local.baseFs ? `${local.baseFs}px` : "16px"}>
          <Slider min={10} max={22} step={1} value={local.baseFs ?? 16} onChange={(v) => update({ baseFs: v })} />
        </Row>
        ...
        <Row label="Border radius" value={local.radius !== undefined ? `${local.radius}px` : ""}>
          <Slider min={0} max={32} step={1} value={local.radius ?? 0} onChange={(v) => update({ radius: v })} />
        </Row>

        <Row label="Shadow" value={local.shadow !== undefined ? local.shadow.toFixed(2) : ""}>
          <Slider min={0} max={1} step={0.05} value={local.shadow ?? 0} onChange={(v) => update({ shadow: v })} />
        </Row>
      </div>
      ...
      <div style={{
        ...
        <button
          onClick={onReset}
          style={{
            fontSize: 11, fontFamily: "var(--df-font-mono)",
            color: "var(--df-text-faint)", padding: "4px 10px",
            background: "transparent", border: "1px solid var(--df-border-subtle)",
            borderRadius: "var(--df-r-sm)", cursor: "pointer",
          }}
        >
          Reset all
        </button>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11 }}>
        ...
        {value && (
          <span style={{ fontFamily: "var(--df-font-mono)", fontSize: 10, color: "var(--df-text-faint)" }}>{value}</span>
        )}
      </div>
      {children}
    </div>
...
function ColorBinding({ value, onChange, placeholder: _placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) {
    ...
    <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
      ...
      {value && (
        <button
          onClick={() => onChange("")}
          title="Clear"
          style={{ color: "var(--df-text-faint)", fontSize: 11, padding: "0 4px", cursor: "pointer" }}
        >✕</button>
      )}
    </div>
  );
...
export function buildEditCss(v: EditOverrides): string {
  ...
  if (v.maxW) {
    parts.push(`body > * { max-width: ${v.maxW}px; margin-left: auto !important; margin-right: auto !important; }`);
  }
  if (v.baseFs) parts.push(`html { font-size: ${v.baseFs}px !important; }`);
  if (v.font) parts.push(`html, body, button, input, textarea, select { font-family: ${v.font} !important; }`);
  if (v.radius !== undefined) {
    parts.push(`:root { --df-edit-radius: ${v.radius}px; }`);
    parts.push(`button, input, textarea, select, img, video, .card, [class*="card"], [class*="btn"] { border-radius: ${v.radius}px !important; }`);
  }
  if (v.shadow !== undefined) {
    const op = v.shadow.toFixed(2);
    parts.push(`[class*="card"], [class*="shadow"], .card { box-shadow: 0 8px 24px rgba(0,0,0,${op}) !important; }`);
  }
  return parts.join("\n");
}
