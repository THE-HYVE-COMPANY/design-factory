...
import { CATEGORY_LABEL, CATEGORY_ORDER, groupByCategory, type Verb } from "@/runtime/verbs/registry";
...
export function CommandLibrary({
  ...
  const filtered = (list: Verb[]) =>
    !q ? list : list.filter((v) =>
      v.label.toLowerCase().includes(q) ||
      v.id.toLowerCase().includes(q) ||
      v.description.toLowerCase().includes(q)
    );

  const totalShown = CATEGORY_ORDER.reduce((acc, cat) => acc + filtered(grouped[cat] ?? []).length, 0);
        ...
        <div style={{
          ...
          <div style={{ position: "relative", marginTop: 18 }}>
            <span aria-hidden style={{
              ...
              color: "var(--df-text-faint)", display: "flex", alignItems: "center",
              pointerEvents: "none",
            }}>
            ...
            <input
              ...
              style={{
                ...
                transition: "border-color 160ms var(--df-ease-out), box-shadow 160ms var(--df-ease-out)",
              }}
              onFocus={(e) => {
            ...
            return (
              <section key={cat} style={{ marginBottom: 24 }}>
                ...
                <div style={{
                  ...
                  {list.map((v) => <VerbCard key={v.id} verb={v} onPick={onPick} />)}
                </div>
              </section>
            );
          })}
  ...
  return (
    <button
      ...
      style={{
        ...
        transition: "transform 160ms var(--df-ease-out), background 140ms var(--df-ease-out), box-shadow 180ms var(--df-ease-out)",
      }}
    >
