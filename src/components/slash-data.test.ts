...
const commands: SlashCommand[] = [
  { id: "polish", trigger: "/polish", label: "Polish", description: "Tighten hierarchy", category: "Commands" },
  { id: "rewrite", trigger: "/rewrite", label: "Rewrite", description: "Fresh approach", category: "Commands" },
  { id: "tweaks", trigger: "/tweaks", label: "Tweaks", description: "Build live controls", category: "Actions" },
  { id: "skill-polish", trigger: "/polish", label: "Polish Skill", description: "Skill collision", category: "Skills" },
  { id: "brand", trigger: "/brand-audit", label: "Brand Audit", description: "Visual identity review", category: "Skills" },
];

describe("slash-data", () => {
  it("initial suggestions hide toolbar-only commands but preserve collisions and order", () => {
    const matches = findMatches("/", commands);
    expect(matches.map((cmd) => cmd.id)).toEqual([
      "polish",
      "rewrite",
      "skill-polish",
      "brand",
  ...
  it("extracts slash and at-trigger tokens at cursor positions", () => {
    ...
    expect(triggerAtCursor("ask @canvas now", "ask @canvas".length)).toEqual({ token: "@canvas", start: 4 });
  });
